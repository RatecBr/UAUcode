...
export default function Dashboard() {
    ...
    const handleUpload = async (e: React.FormEvent) => {
        ...
        if (!editId && !canCreate) {
            alert(`Limite de ${planLimit} experiência(s) atingido. Faça upgrade do seu plano.`);
            return;
        }
        ...
        try {
            ...
            if (editId) {
                const { error } = await supabase.from('targets').update(payload).eq('id', editId);
        ...
        <div style={styles.container}>
            <header style={styles.header}>
                <div style={styles.topRow}>
                    ...
                    <div style={styles.buttons}>
                        ...
                        <button style={styles.iconButton} onClick={() => navigate('/scanner')}>
                            <Camera size={20} />
                        </button>
                        <button style={styles.iconButton} onClick={() => { logout(); navigate('/'); }}>
            ...
            <div style={styles.statsCard}>
                <div style={styles.statItem}>
                    <div style={styles.statValue}>{targets.length}/{planLimit === 999999 ? '∞' : planLimit}</div>
            ...
            <div style={styles.scannerLink}>
                <div style={{ fontSize: '11px', fontWeight: 700, color: 'rgba(255,255,255,0.3)', marginBottom: '10px', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                    Seu link de scanner:
                </div>
                ...
                <div style={styles.modal}>
                    <div style={styles.modalContent}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '24px' }}>
                            ...
                            <button onClick={resetForm} style={{ background: 'rgba(255,255,255,0.05)', border: 'none', cursor: 'pointer', padding: '10px', borderRadius: '14px', color: '#fff' }}>
                                <X size={20} />
                            </button>
                        </div>
                        ...
                        <form onSubmit={handleUpload} style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
                                    ...
                                    {['video', 'audio', '3d'].map(type => (
                                        <button
                                            ...
                                            style={{
                                                ...
                                                borderRadius: '14px',
                                                border: '2px solid',
                                                borderColor: contentType === type ? '#00ff9d' : 'rgba(255,255,255,0.05)',
                                                backgroundColor: contentType === type ? 'rgba(0,255,157,0.1)' : 'rgba(255,255,255,0.02)',
                                                color: contentType === type ? '#00ff9d' : 'rgba(255,255,255,0.3)',
                                                fontSize: '12px',
                                                fontWeight: 700,
                                                cursor: 'pointer',
                                                transition: 'all 0.2s'
                            ...
                            <div>
                                <label style={styles.fieldLabel}>IMAGEM ALVO (MARCADOR AR)</label>
                                <div style={styles.dropzone}>
                                    {targetPreview ? (
                                        ...
                                        <div style={{ display: 'flex', gap: '12px', width: '100%' }}>
                                            <button
                                                type="button"
                                                onClick={() => { setCaptureMode('photo'); setShowCapture(true); }}
                                            ...
                                            <label style={styles.modernUploadBtn}>
                                                ...
                                                <input type="file" accept="image/*" onChange={e => handleFileChange(e.target.files?.[0] || null, 'target')} style={{ display: 'none' }} />
                                            </label>
                                        </div>
                                    )}
                                </div>
                                    ...
                                    {contentPreview ? (
                                        ...
                                        <div style={{ display: 'flex', gap: '12px', width: '100%' }}>
                                            {contentType !== '3d' && (
                                                <button
                                                    type="button"
                                                    onClick={() => { setCaptureMode(contentType as 'video' | 'audio'); setShowCapture(true); }}
                                                    style={styles.modernUploadBtn}
                                                >
                                                    {contentType === 'video' ? <Video size={20} color="#00ff9d" /> : <Mic size={20} color="#00ff9d" />} GRAVAR
                                                </button>
                                            )}
                                            <label style={styles.modernUploadBtn}>
                                                <ImageIcon size={20} color="#00ff9d" /> ARQUIVO
                                                <input type="file" accept={contentType === 'video' ? 'video/*' : contentType === 'audio' ? 'audio/*' : '.glb,.gltf'} onChange={e => handleFileChange(e.target.files?.[0] || null, 'content')} style={{ display: 'none' }} />
                                            </label>
                                        </div>
                                    )}
                                </div>
                            ...
                            <button type="submit" style={styles.primaryBtn} disabled={uploading}>
                                {uploading ? <Loader2 size={20} className="spin" /> : (editId ? 'SALVAR ALTERAÇÕES' : 'CRIAR AGORA')}
                            </button>
                        </form>
                    </div>
                </div>
            ...
            {showCapture && (
                <MediaCapture
                    mode={captureMode}
                    onCapture={(file) => {
                        handleFileChange(file, captureMode === 'photo' ? 'target' : 'content');
            ...
            <div style={styles.card}>
                ...
                {targets.length === 0 ? (
                    <div style={{ padding: '60px 40px', textAlign: 'center', backgroundColor: 'rgba(255,255,255,0.02)', borderRadius: '24px', border: '1px dashed rgba(255,255,255,0.1)' }}>
                        <ImageIcon size={48} style={{ opacity: 0.2, marginBottom: '16px' }} />
                        <div style={{ color: 'rgba(255,255,255,0.4)', fontSize: '15px' }}>Nada por aqui ainda. Clique no + para começar!</div>
                    </div>
                ) : (
                    targets.map(t => (
                        <div key={t.id} style={styles.listItem}>
                            ...
                            <div style={styles.itemInfo}>
                                ...
                                <div style={styles.itemMeta}>
                                    <span style={{ backgroundColor: 'rgba(255,255,255,0.05)', padding: '2px 8px', borderRadius: '6px', fontSize: '10px', fontWeight: 700, textTransform: 'uppercase' }}>{t.content_type}</span>
                                    <span style={{ display: 'flex', alignItems: 'center', gap: '4px' }}><BarChart3 size={12} /> {t.scan_count || 0}</span>
                                </div>
                            </div>
                            <div style={styles.itemActions}>
                                <button style={styles.actionBtn} onClick={() => setShowQR(t.id)}><QrCode size={18} /></button>
                                <button style={styles.actionBtn} onClick={() => startEdit(t)}><Edit2 size={18} /></button>
                                <button style={{ ...styles.actionBtn, color: '#ff4757' }} onClick={() => handleDelete(t.id)}><Trash2 size={18} /></button>
                            </div>
                        </div>
                    ))
                )}
            ...
            {showQR && (
                <div style={styles.modal} onClick={() => setShowQR(null)}>
                    <div style={{ ...styles.modalContent, textAlign: 'center' }}>
                        <h3 style={{ margin: '0 0 24px', fontSize: '20px', fontWeight: 800 }}>QR Code da Experiência</h3>
                        <div style={{ backgroundColor: '#fff', padding: '20px', borderRadius: '24px', display: 'inline-block' }}>
                            <QRCodeGenerator
                                url={`${scannerUrl}/${showQR}`}
                                name={targets.find(t => t.id === showQR)?.name || ''}
                            />
                        </div>
                        <button onClick={() => setShowQR(null)} style={{ ...styles.primaryBtn, marginTop: '24px', backgroundColor: 'rgba(255,255,255,0.1)', color: '#fff' }}>Fechar</button>
                    </div>
                </div>
            )}
            ...
            {showSettings && (
                <div style={styles.modal} onClick={() => setShowSettings(false)}>
                    <div style={styles.modalContent} onClick={e => e.stopPropagation()}>
                        <h3 style={{ margin: '0 0 24px', fontSize: '20px', fontWeight: 800 }}>Configurações</h3>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                            <button onClick={updateSlug} style={styles.primaryBtn}>ATUALIZAR SLUG</button>
                            <button onClick={() => setShowSettings(false)} style={{ ...styles.primaryBtn, backgroundColor: 'transparent', color: '#fff', border: '1px solid rgba(255,255,255,0.1)' }}>CANCELAR</button>
                        </div>
                    </div>
                </div>
            )}
